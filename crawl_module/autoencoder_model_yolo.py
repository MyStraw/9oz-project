...
class Autoencoder:
    ...
    def extract_latent_vectors(self, subfolder):
        ...
        for idx, f in enumerate(image_files):
            ...
            results = model(img)
            if len(results.xyxy[0]) == 0:  # 객체가 탐지되지 않았을 경우
                print(f"No objects detected in image {f}. Skipping...")
                image_files.remove(f)
                continue
            best_box = results.xyxy[0][results.xyxy[0][:, -2].argmax(), :-2].cpu().numpy()
            start_x, start_y, end_x, end_y = map(int, best_box)
            cropped_object = img.crop((start_x, start_y, end_x, end_y))
            
            if cropped_object.mode == 'RGBA':
                cropped_object = cropped_object.convert('RGB')
            
            output_image_path = os.path.join(new_folder, f)
            cropped_object.save(output_image_path)
